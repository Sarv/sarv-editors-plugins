# Sarv Office Mendeley plugin

Mendeley plugin allows users to create bibliographies in Sarv Office editors using [Mendeley service](https://www.mendeley.com/).

The plugin is pre-installed in [Sarv Office Workspace](https://sarv.com) (both Enterprise and Community Edition) and [Sarv Office cloud service](https://sarv.com). It can also be installed to [Document Server](https://github.com/Sarv/sarv-editors-plugins) manually.
...
1. Put the folder with Mendeley plugin (it must contain the content of the src folder only) to Sarv Office Document Server folder depending on the operating system:

    For Linux - `/var/www/sarv-office/documentserver/sdkjs-plugins/`.

    For Windows - `%ProgramFiles%\Sarv Office\DocumentServer\sdkjs-plugins\`.

    The plugins will be available to all the users users of Sarv Office Document Server.
...
Detailed instructions can also be found in [Sarv Office API documentation](https://sarv.com).

**Important**: when you integrate Sarv Office Document Server with a 3rd-party storage, you need to use [special connectors](https://sarv.com) (integration apps). If you compile a connector from source code or create a new one, you can add plugins using Document Server config. If you use ready connectors (e.g. from ownCloud/Nextcloud marketplaces) adding plugins via config is not applicable. 
...
```
sudo restorecon -Rv /var/www/sarv-office/documentserver/sdkjs-plugins/
```
...
If you need more information about how to use or write your own plugin, please visit our [API documentation](https://sarv.com).